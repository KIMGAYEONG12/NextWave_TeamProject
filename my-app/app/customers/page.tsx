        ...
        {tabs.map((t) => (
          <button
            ...
            className={`rounded-xl px-4 py-2 text-sm font-semibold transition ${
              tab === t ? "bg-brand-600 text-white shadow-card" : "bg-white text-black hover:bg-slate-50"
            }`}
          >
            {t}
          </button>
      ...
      {tab === "리뷰 관리" && (
        <>
          <div className="mb-5 grid grid-cols-2 gap-4 sm:grid-cols-4">
            <div className="card p-4">
              <p className="text-xs text-black">전체 리뷰</p>
              <p className="mt-1 text-xl font-bold text-black">{reviewStats.total}개</p>
            </div>
            <div className="card p-4">
              <p className="text-xs text-black">평균 평점</p>
            ...
            <div className="card p-4">
              <p className="text-xs text-black">답변 비율</p>
              <p className="mt-1 text-xl font-bold text-emerald-600">{reviewStats.replyRate}%</p>
            </div>
            <div className="card p-4">
              <p className="text-xs text-black">신고/차단 요청</p>
                ...
                <div key={r.id} className="flex items-start gap-3 rounded-xl border border-slate-100 p-4">
                  ...
                  <div className="flex-1">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="text-sm font-semibold text-black">{r.customer}</span>
                      <span className="text-xs text-black">{r.grade}</span>
                    ...
                    <p className="mt-1 text-sm text-black">{r.text}</p>
                    <p className="mt-1 text-xs text-black">{r.date}</p>
                  </div>
      ...
      {tab === "신고/차단 관리" && (
        <div className="card p-5">
          ...
          {reportedReviews.length === 0 ? (
            <p className="py-10 text-center text-sm text-black">신고 접수된 항목이 없습니다.</p>
          ) : (
            <div className="space-y-3">
                ...
                <div key={r.id} className="flex items-center justify-between rounded-xl border border-red-100 bg-red-50 p-4">
                  <div>
                    <p className="text-sm font-semibold text-black">
                      {r.customer} · <MessageSquare size={12} className="mb-0.5 inline text-black" /> {r.text}
                    </p>
                    <p className="mt-1 text-xs text-black">{r.date}</p>
                  </div>
