...
const insights = [
  { icon: "reservation" as const, text: "금요일 오후 2-4시에 매출이 가장 높을 것으로 예상돼요.", sub: "지난 4주 데이터 기반 예측" },
  { icon: "sales" as const, text: "딸기 음료의 판매가 증가할 것으로 예상돼요.", sub: "최근 3주 판매 데이터 분석" },
  { icon: "ai" as const, text: "월요일 오전 매출이 낮아요.", sub: "프로모션 추천 드립니다." },
];
  ...
  return (
    <div className="p-6 md:p-8 max-w-[1400px] mx-auto">
          ...
          <Card key={f.name}>
            <div className="flex items-center justify-between mb-2">
              <span className="flex items-center gap-1.5 text-sm font-semibold text-ink-800">
                <span className="text-[10px] bg-brand-50 text-brand-600 rounded px-1.5 py-0.5">AI</span>
                {f.name}
              </span>
            </div>
            <p className="text-xs text-ink-500 mb-4 h-8">{f.desc}</p>
            <div className="flex items-center justify-between">
              <span className="text-xs text-ink-500">{f.enabled ? "활성화됨" : "비활성화"}</span>
              <button
                onClick={() =>
                  setFeatures((prev) =>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-5">
        <Card>
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-semibold text-ink-800">AI 분석 리포트</h3>
          </div>
          <div className="mb-4">
            <StoreTabs
              tabs={[
          ...
          <div className="flex items-center justify-between mb-1">
            <div>
              ...
              <p className="text-2xl font-bold text-ink-900">
                ₩7,850,000 <span className="text-emerald-600 text-sm font-medium">▲ 12.5%</span>
              </p>
              <p className="text-xs text-ink-400">지난 주 대비</p>
            </div>
          </div>
          <LineChart data={salesTrend.map((s) => ({ day: s.day.split("(")[0], value: s.value * 8.4 }))} />
        </Card>

        <Card>
          <h3 className="text-sm font-semibold text-ink-800 mb-4">AI 추천 인사이트</h3>
          ...
          <a href="#" className="mt-4 inline-block text-xs text-brand-600 font-medium">전체 리포트 보기 →</a>
        </Card>
      </div>
    </div>
  );
