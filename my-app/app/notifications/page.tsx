...
export default function NotificationsPage() {
  ...
  const filtered = notifications.filter(
    (n) => tab === "all" || typeToTab[n.type] === tab,
  );
  ...
  return (
    <div className="p-6 md:p-8 max-w-[1400px] mx-auto">
      <PageHeader
        title="알림 센터"
        subtitle="매장 운영과 관련된 알림을 확인하고 관리할 수 있습니다."
      />
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-5">
        <Card className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xl font-bold text-ink-900">28</p>
            <p className="text-xs text-ink-400">전체 알림</p>
          </div>
        </Card>
        <Card className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xl font-bold text-ink-900">3</p>
            <p className="text-xs text-ink-400">확인 필요</p>
          </div>
        </Card>
        <Card className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-red-50 text-red-600 flex items-center justify-center">
            <Icon name="aiSettings" className="w-5 h-5" />
          </div>
          <div>
            <p className="text-xl font-bold text-ink-900">5</p>
            <p className="text-xs text-ink-400">중요 알림</p>
          </div>
        </Card>
        <Card className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center">
            <Icon name="reservation" className="w-5 h-5" />
          </div>
          <div>
            <p className="text-xl font-bold text-ink-900">12</p>
            <p className="text-xs text-ink-400">오늘 알림</p>
          </div>
        </Card>
      </div>
        ...
        <Card padded={false}>
          ...
          <div className="divide-y divide-ink-50">
            {filtered.map((n, i) => (
              <div
                key={i}
                className={`flex items-center gap-3 px-5 py-3.5 ${!n.read ? "bg-brand-50/30" : ""}`}
              >
          ...
          <div className="space-y-3.5">
            {toggles.map((t) => (
              <div key={t.key} className="flex items-center justify-between">
                ...
                >
                  <span
                    className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-transform ${enabled[t.key] ? "translate-x-4" : "translate-x-0.5"}`}
                  />
                </button>
              </div>
            ))}
