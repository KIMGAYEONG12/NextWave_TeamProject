...
export default function Topbar() {
  ...
  return (
    <div className="flex items-center justify-end gap-3 border-b border-slate-200 bg-white/80 px-4 py-3 backdrop-blur lg:px-8">
      <div className="hidden items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-sm text-black sm:flex">
      ...
      <Link
        href="/notifications"
        className="relative flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 text-black hover:bg-slate-50"
      >
        <Bell size={17} />
      ...
      <div className="relative">
        <button
          ...
          className="flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-sm font-medium text-black hover:bg-slate-50"
        >
            ...
            {stores.map((s) => (
              <button
                ...
                className={`w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-50 ${
                  s === selected ? "font-semibold text-brand-600" : "text-black"
                }`}
              >
                {s}
              </button>
